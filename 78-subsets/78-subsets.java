class Solution {
    public List<List<Integer>> subsets(int[] nums) {
        // Arrays.sort(nums);                                    //sorting the array, so that, the duplicate elements are adjacent to each other. Not required in this question.
    ...
    public static void findSubsets(int start, int[] nums, List<Integer> individualSubset, List<List<Integer>> answerList){
        ...
        for(int i = start; i<nums.length; i++){                    //start = 0.
            ...
            findSubsets(i+1, nums, individualSubset, answerList);  //starting at (i+1), because, the number at (i) has been already added in line 13.
        ...
        }                                                          //0 to (nums.length-1). Thus, the iteration will stop as it will come out of for loop. To avoid that, we do line 15.
    }
}