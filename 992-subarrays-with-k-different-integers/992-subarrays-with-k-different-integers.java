...
//a condition that checks the credibility.		        
		        if(memo.get(discardInt) == 0)     //if frequency of char is 0, it has to be removed from the HashMap.
 		        {
		            memo.remove(discardInt);      //since we are completely removing a char(not decrementing it).
